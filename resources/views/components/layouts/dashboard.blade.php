...
{{-- Layout khusus untuk halaman dashboard (TIKKIM & Seksi) --}}
<!DOCTYPE html>
...
<head>
    ...
    <style>
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        /* Sidebar */
        #sidebar { transition: transform 0.25s cubic-bezier(.4,0,.2,1), width 0.25s cubic-bezier(.4,0,.2,1); }
...
<body class="h-full bg-slate-50" x-data="{ sidebarCollapsed: false, mobileOpen: false }"
      @keydown.escape.window="mobileOpen = false">

    {{-- SIDEBAR OVERLAY (mobile) --}}
    <div id="sidebar-overlay"
         :class="mobileOpen ? 'show' : ''"
    ...
    <div class="flex h-full min-h-screen">
        ...
        <aside id="sidebar"
               ...
               :class="[sidebarCollapsed ? 'collapsed' : '', mobileOpen ? 'mobile-open' : '']">
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
                ...
                {{-- Utama --}}
                <div class="nav-section-label text-[10px] font-bold text-slate-600 uppercase tracking-widest px-3 mb-2 mt-1">
                    Utama
                </div>
                ...
                {{-- Pengaduan --}}
                <div class="nav-section-label text-[10px] font-bold text-slate-600 uppercase tracking-widest px-3 mb-2 mt-4">
                    Pengaduan
                </div>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="nav-item w-full text-left hover:!bg-red-900/30 hover:!text-red-400">
                        <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75"
            ...
            <div class="border-t border-slate-800 p-4 flex-shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div class="user-meta min-w-0">
                        ...
                        <div class="text-slate-400 text-[10px] truncate uppercase tracking-wide">
        ...
        <div id="main-content" class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            {{-- TOPBAR --}}
            <header class="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-5 flex-shrink-0 z-20 sticky top-0">
                <div class="flex items-center gap-3">
                    {{-- Toggle sidebar (desktop) --}}
                    <button @click="sidebarCollapsed = !sidebarCollapsed"
                            class="hidden md:flex w-8 h-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition">
                    ...
                    <div class="hidden sm:block">
                        <h1 class="text-sm font-semibold text-slate-800">{{ $header ?? 'Dashboard' }}</h1>
                ...
                <div class="flex items-center gap-2">
                    {{-- Notif placeholder --}}
                    <button class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 transition relative">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                        </svg>
                        @if (isset($slaOver) && $slaOver > 0)
