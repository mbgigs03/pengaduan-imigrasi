    ...
    <div class="flex h-full min-h-screen">
        ...
        <aside id="sidebar"
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
                ...
                {{-- Hanya tampil jika role adalah tikkim atau super_admin --}}
                @if (Auth::user()->profile && in_array(Auth::user()->profile->role, ['tikkim', 'kakanim']))
                ...
                {{-- KHUSUS TIKKIM: Kelola FAQ --}}
                @if (auth()->user()->profile && auth()->user()->profile->role === 'tikkim')
                    <a href="{{ route('faq.index') }}"
                       class="nav-item {{ request()->routeIs('faq.*') ? 'active' : '' }}">
                        ...
                        <span class="nav-label">Kelola FAQ</span>
                    </a>
                @endif
