        ...
        >
            ...
            <div class="p-6 space-y-5">
                ...
                <div x-show="statusPilih === 'selesai'" x-transition>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Foto Bukti Penyelesaian</label>
                    
                    {{-- Container Preview --}}
                    <div x-data="{ previewUrl: null }" class="space-y-3">
                        <label class="block border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center cursor-pointer hover:border-blue-500 hover:bg-blue-50/50 transition"
                               x-show="!previewUrl">
                            ...
                            <div class="font-medium text-slate-700">Upload Foto Bukti</div>
                            <div class="text-xs text-slate-500 mt-1">JPG, PNG maksimal 5MB</div>
                            <input type="file" name="bukti_gambar" accept="image/*" class="hidden" 
                                   @change="const file = $event.target.files[0]; if(file) { const reader = new FileReader(); reader.onload = (e) => previewUrl = e.target.result; reader.readAsDataURL(file); }">
                        </label>

                        {{-- Area Preview --}}
                        <div x-show="previewUrl" class="relative group w-full h-48 rounded-xl overflow-hidden border border-slate-200">
                            <img :src="previewUrl" class="w-full h-full object-cover">
                            <button type="button" @click="previewUrl = null; $el.closest('div').querySelector('input').value = ''" 
